...
export function exportHTMLReport() {
    ...
    if (state.auditData) {
        ...
        if (hd && !hd.error) {
            auditHtml += `<h2>HTTP Security Headers — Grade: <span style="color:${{ A: '#00ff88', B: '#44cc88', C: '#ffaa00', D: '#ff6600', F: '#ff0033' }[hd.grade] || '#888'}">${hd.grade}</span> (${hd.score}/100)</h2>${hd.missing?.length ? '<p style="color:#ff0033;margin-bottom:6px">Missing: ' + hd.missing.map(h => `<code>${h.header}</code>`).join(', ') + '</p>' : ''}`;
        }
